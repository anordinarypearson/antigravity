...
export function SignUpContent() {
    const { signUpWithEmail, sendVerificationEmail, signInWithGoogle, signInWithGithub, signInWithFacebook, signInWithTwitter } = useAuth();
    ...
    const getPasswordStrength = (pass: string) => {
        ...
        const colors = ["", "bg-destructive", "bg-yellow-500", "bg-green-500", "bg-emerald-600"];
    ...
    const handleEmailSignUp = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (user) {
                // Wait a moment for user to be fully created in Firebase
                ...
                try {
                    ...
                    // Account created but email failed - still show as partial success
                    setError(
                        `Account created, but failed to send verification email: ${emailError.message}. You can resend it by clicking the button below.`
                    );
                }
            }
        } catch (err: any) {
                        ...
                        <AnimatePresence mode="wait">
                            {error && (
                                <motion.div
                                    ...
                                    className="bg-destructive/10 border border-destructive/20 text-destructive text-sm p-3 rounded-lg flex flex-col gap-2"
                                >
                                        ...
                                        <button
                                            onClick={async () => {
                                                try {
                                                    await sendVerificationEmail();
                                                    setError(null);
                                                    setSuccessMessage(`Verification email sent to ${email}! Check your inbox.`);
                                                } catch (err: any) {
                                                    setError(err.message || 'Failed to resend verification email');
                                                }
                                            }}
                                            className="text-xs font-bold hover:underline text-left pl-5 opacity-80 hover:opacity-100"
                                        >
                                            Click here to try sending again
                                        </button>
                                    )}
                                </motion.div>
                            )}
                            {successMessage && (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, scale: 1 }}
                                    exit={{ opacity: 0, scale: 0.95 }}
                                    className="bg-primary/10 border border-primary/20 text-primary text-sm p-3 rounded-lg flex flex-col gap-2"
                                >
                                    <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        onClick={async () => {
                                            try {
                                                await sendVerificationEmail();
                                                setSuccessMessage(`Verification email resent to ${email}! Check your inbox.`);
                                            } catch (err: any) {
                                                setError(err.message || 'Failed to resend verification email');
                                                setSuccessMessage(null);
                                            }
                                ...
                                {password.length > 0 && (
                                    <div className="space-y-1.5 px-1 pt-1">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className={`text-[10px] font-bold uppercase ${strength.score >= 3 ? 'text-green-500' : strength.score >= 2 ? 'text-yellow-500' : 'text-destructive'}`}>
                                                {strength.label}
                                            </span>
                                        </div>
